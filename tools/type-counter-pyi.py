...
class Analyzer(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node):
        if into_file_name(self.metrics.file_name) in self.functions.keys(
        ) and node.name in self.functions[into_file_name(
    ...
    #def visit_Assign(self, node):
        '''
        if into_file_name(self.metrics.file_name) in self.assignments.keys():
            for tgt in node.ktargets:
                if hasattr(tgt, 'id'):
                    if tgt.id in self.assignments[into_file_name(
                            self.metrics.file_name)]:
                        self.metrics.assignments += 1'''

    def visit_AnnAssign(self, node):
        if into_file_name(self.metrics.file_name) in self.assignments.keys(
        ) and node.target.id in self.assignments[into_file_name(
                self.metrics.file_name)]:
...
if __name__ == "__main__":
    ...
    for file in list_of_files:
        file_name = into_file_name(file)
        if file_name in functions_analyzed.keys(
        ) or file_name in assignments_analyzed.keys():
            a = Analyzer(file, functions_analyzed, assignments_analyzed)
            if os.path.exists(
                    "/home/daniel/Documents/College/Semester7/Research/type-analyzer/allrepo/stripped/"
                    + file):
                files_found += 1
                try:
                    f = open(
